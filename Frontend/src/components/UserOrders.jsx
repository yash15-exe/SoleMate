...
function UserOrders() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.post('http://localhost:8000/api/order/getAllUserOrders', { userId });
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      await axios.post('http://localhost:8000/api/order/requestOrderForCancellation', { orderId });
  ...
  const handleRateProduct = async (productId, rating) => {
    try {
      const response = await axios.post('http://localhost:8000/api/products/rateProduct', { productId, rating });
