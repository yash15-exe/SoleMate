...
function AllOrders() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.post('http://localhost:8000/api/order/getAllAdminOrders'); // Adjust the endpoint URL according to your setup
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      await axios.post('http://localhost:8000/api/order/updateOrderStatus', { newStatus, orderId });
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    if (confirmed) {
      try {
        await axios.post('http://localhost:8000/api/order/cancelOrder', { orderId });
        setOrders(prevOrders =>
          prevOrders.map(order =>
