...
function AllProducts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.post('http://localhost:8000/api/products/getAllProducts'); // Adjust the endpoint URL according to your setup
  ...
  const handleUnlistProduct = async (productId) => {
    ...
    if (confirmed) {
      try {
        await axios.post('http://localhost:8000/api/products/unlistProducts', { productId }); // Adjust the endpoint URL according to your setup
